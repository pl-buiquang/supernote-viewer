...
export class TauriPlatform implements IPlatform {
  ...
  public isMobile = platform() === 'android' || platform() === 'ios';
  ...
  // Read File
  async readFile(path: string, localAppData = false): Promise<ArrayBuffer> {
  ...
  async exists(path: string, localAppData = false): Promise<boolean> {
    this.logger.logInfo('Tauri: Checking if file exists', path);
    const options = localAppData ? { baseDir: BaseDirectory.AppLocalData } : {};
    return await tauriExists(path, options);
  }

  async deleteFile(path: string, localAppData = false): Promise<void> {
